...
await page.goto(`https://www.airbnb.fr/users/show/${userId}`, {
  ...
  try {
    ...
    await page.evaluate(() => {
        ...
        const targetButton = buttons.find(button => button.innerText.includes('Afficher les'));
  ...
  while (true) {
    try {
      ...
      let result = await page.evaluate(() => {
          const buttons = Array.from(document.querySelectorAll('button'));
          const targetButton = buttons.find(button => button.innerText.includes("Afficher d'autres annonces"));
          if (targetButton) {
            targetButton.click();
            return true;
          } else {
      ...
      console.log(error)
      console.log("Le bouton pour afficher les autre annonces n'a pas été trouvé");
  ...
  function getFirstHalfOfString(str) {
    ...
    const titleElement = cardContainer ? cardContainer.querySelector('[data-testid="listing-card-title"]') : null;
    const subtitleElement = cardContainer ? cardContainer.querySelector('[data-testid="listing-card-subtitle"] span:nth-child(1)') : null;

    return {
      url: anchor.href.split('?')[0],
      image: img ? img.src : null,
      title: titleElement ? titleElement.textContent.trim() : null,
      subtitle: subtitleElement ? getFirstHalfOfString(subtitleElement.textContent.trim()) : null
    };
  })
);
